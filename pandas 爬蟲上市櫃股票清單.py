import pandas as pd
import requests
#加入headers, user-agent=瀏覽器，代理你去對方發送可靠請求，若為python腳本爬取對方資料會被退回
headers = {
...
x['代號'] = x['有價證券代號及名稱'].apply(lambda x: x.split()[0]) #用空白分開的資料取第一個
#使用split方法，以兩個空白切割字串，並取切割完後第一個，儲存至新增的股票名稱欄位
x['股票名稱'] = x['有價證券代號及名稱'].apply(lambda x: x.split()[-1]) #用空白分開的資料取最後一個
